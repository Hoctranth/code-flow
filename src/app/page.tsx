        ...
        <div className="w-[400px] h-full flex flex-col bg-panel-bg border-r border-border-main shrink-0 z-10 shadow-2xl relative transition-colors duration-300">
          <div className="px-4 py-3 bg-surface-bg border-b border-border-main flex items-center justify-between transition-colors duration-300">
            ...
            <div className="flex gap-2">
              <button onClick={() => setShowEditor(false)} className="text-txt-muted hover:text-txt-primary text-xs px-2 py-1 bg-btn-bg hover:bg-btn-hover rounded transition-colors duration-300">
                Hide
              </button>
            </div>
          </div>
      ...
      <div className="flex-1 h-full flex flex-col relative bg-app-bg transition-colors duration-300">
        ...
        <div className="absolute top-4 left-4 z-20 flex gap-2">
          ...
          <div className="flex bg-btn-bg border border-border-main rounded-md shadow-lg overflow-hidden items-center">
            <span className="text-[10px] uppercase font-bold text-txt-muted px-2">Theme:</span>
            <select
              value={theme}
            ...
            >
              <option value="dark" className="bg-panel-bg">Dark</option>
              <option value="light" className="bg-panel-bg">Light</option>
              <option value="dracula" className="bg-panel-bg">Dracula</option>
            </select>
            
            <div className="w-[1px] h-4 bg-border-light mx-1"></div>

            <span className="text-[10px] uppercase font-bold text-txt-muted px-2">Layout:</span>
            <select
              value={algorithm}
              onChange={(e) => setAlgorithm(e.target.value as LayoutAlgorithm)}
              className="bg-transparent text-xs font-medium text-txt-primary py-1.5 px-2 outline-none cursor-pointer hover:bg-btn-hover"
            >
              <option value="dagre" className="bg-panel-bg">Dagre</option>
              <option value="elk" className="bg-panel-bg">ELK</option>
              <option value="force" className="bg-panel-bg">Force</option>
            </select>
